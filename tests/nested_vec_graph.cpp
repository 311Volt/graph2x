...
namespace {
	...
	TEST(nested_vec_graph, empty_graph_should_be_empty) {
		g2x::nested_vec_graph graph{3, std::vector<std::pair<int,int>>{}};
		EXPECT_EQ(g2x::num_edges(graph), 0);
	}

	TEST(nested_vec_graph, adjacency_cardinality_should_match_edge_list) {
		g2x::nested_vec_graph graph{4, std::vector<std::pair<int,int>>{
					{0, 1},
					{1, 2},
					{1, 3},
					{2, 0}
	...
	TEST(nested_vec_graph, undirected_all_edges_should_not_yield_duplicates) {
		g2x::nested_vec_graph graph{3, std::vector<std::pair<int,int>>{
					{0, 1},
					{1, 2},
					{2, 0}
	...
	TEST(nested_vec_graph, undirected_should_handle_multiple_loops) {
		g2x::nested_vec_graph graph{4, std::vector<std::pair<int,int>>{
					{0, 1},
					{1, 2},
					{2, 0},
	...
	TEST(nested_vec_graph, undirected_adjacency_relation_should_be_symmetric) {
		g2x::nested_vec_graph graph{3, std::vector<std::pair<int,int>>{
						{0, 1},
						{1, 2},
						{2, 0}
	...
	TEST(nested_vec_graph, directed_adjacency_relation_should_be_asymmetric) {
		g2x::nested_vec_digraph graph{3, std::vector<std::pair<int,int>>{
					{0, 1},
					{1, 2},
					{2, 0}
		}};
